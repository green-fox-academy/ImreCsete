...
void Bluetooth_Config(UART_HandleTypeDef *huart) {
	...
	UartTransmit.Pin 		= GPIO_PIN_6;
	UartTransmit.Mode       = GPIO_MODE_AF_PP;
	UartTransmit.Pull		= GPIO_PULLUP;
	UartTransmit.Speed 		= GPIO_SPEED_FAST;
	UartTransmit.Alternate  = GPIO_AF8_USART6;

	HAL_GPIO_Init(GPIOC, &BluetoothTX);

	UartRecieve.Pin 		= GPIO_PIN_7;
	UartRecieve.Mode        = GPIO_MODE_AF_PP;
	UartRecieve.Speed 		= GPIO_SPEED_FAST;
	UartRecieve.Alternate   = GPIO_AF8_USART6;

	HAL_GPIO_Init(GPIOC, &BluetoothRX);
...
/*---Saves the Receive Data Register bit by bit in the character array for the tokenizer-------------*/
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	if (huart->Instance == USART1) {
		RDR_Buffering();
		HAL_UART_Receive_IT(&UartHandle, &UART_IT_Data, 1);		// Reinit Uart interrupt for the new bit.
	} else {
		RDR_Buffering();
		HAL_UART_Receive_IT(&Bluetooth, &UART_IT_Data, 1);		// Reinit Bluetooth interrupt for the new bit.
	}
}
